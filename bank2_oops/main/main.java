      ...
      switch (selectedOption) {
        ...
        case 5:
          ...
          if (map.containsKey(userId) && map.get(userId)) {
            System.out.println(
              "You have already raised a request and it is already approved"
            );
          } else if (map.containsKey(userId) && !map.get(userId)) {
            System.out.println(
              "You have already raised a request and it is pending for approval"
            );
          } else {
  ...
  private void fundTransfer(User userDetails) {
    ...
    if (user != null) {
      ...
      if (userAccountBalance >= amount) {
        boolean result = userService.transferAmount(
        ...
        System.out.println(
          "Your balance is insufficient: " + userAccountBalance
        );
      }
    } else {
